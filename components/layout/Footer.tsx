...
const WA_MESSAGE = "Hola, me gustaría conocer más información sobre la campaña.";

export async function Footer() {
  const candidate = await getCandidate();
  const waUrl = buildWhatsAppUrl(candidate.phone, WA_MESSAGE);
    ...
    <footer className="border-t border-line bg-brand-dark py-14 text-white sm:py-16">
      <Container className="grid gap-10 md:grid-cols-2 xl:grid-cols-[1.2fr_0.8fr_0.8fr]">
        ...
        <div className="space-y-4 md:col-span-2 xl:col-span-1">
          ...
          {/* Social icons */}
          <SocialLinks links={candidate.socialLinks} variant="light" className="mt-2" />
        </div>
        ...
        <div className="md:max-w-xs">
          ...
          <div className="space-y-3 text-sm text-white/80">
            ...
            <WhatsAppLink
              href={waUrl}
              source="footer"
              className="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-2 text-xs font-semibold text-white transition hover:border-white/50 hover:text-white"
