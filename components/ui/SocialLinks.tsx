...
type SocialLinksProps = {
  links: SocialLink[];
  className?: string;
  /** dark = teal-on-light (footer brand col, hero card); light = white-on-dark */
  variant?: "dark" | "light";
};
...
export function SocialLinks({ links, className, variant = "dark" }: SocialLinksProps) {
      ...
      {links.map((link) => (
        <a
          ...
          className={cn(
            ...
            variant === "dark"
              ? "bg-surface-alt text-slate-600 hover:bg-brand-soft hover:text-brand"
